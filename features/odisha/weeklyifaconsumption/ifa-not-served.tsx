...
type TabletsNotServedFormInputs = {
...
export default function MealsNotServed() {
  const {
    ...
    control,
    formState: {errors},
  } = useForm<TabletsNotServedFormInputs>({
    mode: 'onChange',
    defaultValues: {
  ...
  async function onSubmit(data: TabletsNotServedFormInputs) {
    const smsAvailable = await SMS.isAvailableAsync()
    if (smsAvailable) {
      SMS.sendSMSAsync(
        ['15544'],
        `MDMODI T${data.totalNumOfStudentsPresent}N${data.reason}`
      )
    } else {
  ...
  const reasons = [
    ...
    {
      value: '2',
      label: 'Available tablet expired',
    },
    {
      value: '3',
      label: 'Student Resistance',
    },
    {
      value: '4',
      label: 'Teacher/SMC non co-operation',
    },
    {
      value: '5',
      label: 'Others',
